...
import { checkConnectionsService } from "../services/CheckConnectionsService";
...
export default class API_DB_Controller {
    async executeHandler(req: Request, res: Response, serviceClass: any) {
        try {
            ...
            const result = await serviceInstance.execute(req.query, req.ip);
    ...
    handleCheckConnections = async (req: Request, res: Response) => {
        this.executeHandler(req, res, checkConnectionsService)
    }
